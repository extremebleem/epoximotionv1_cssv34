...
bool ImGui::KeyButton( const char* label, int* key, const ImVec2& size_arg )
{
	...
	const char* active_text = nullptr;
	...
	ImVec2 Josk((frame_bb.Max.x - frame_bb.Min.x)/2, frame_bb.Max.y);
	const ImU32 col = GetColorU32( ( hovered && held ) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg );
	RenderFrame(frame_bb.Min, ImVec2(frame_bb.Min.x + 40, frame_bb.Max.y), col, true, 5.f ); // main frame

	if( active_text ) 
	{
		label_size = CalcTextSize( active_text, NULL, true );
		RenderTextClipped( frame_bb.Min, ImVec2(frame_bb.Min.x + 40, frame_bb.Max.y), active_text, NULL, &label_size, ImGuiAlign_Center | ImGuiAlign_VCenter );
	}
	if(label_size.x > 0)
		RenderText( ImVec2( frame_bb.Min.x + 45, frame_bb.Min.y), label );
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	// Draw frame
	RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, 10);
	...
	ImRect grab_bb;
	if (is_horizontal)
		grab_bb = ImRect(frame_bb.Min, ImVec2(grab_pos + grab_sz*0.5f, frame_bb.Max.y)); // normal
	else
		grab_bb = ImRect(frame_bb.Min, ImVec2(frame_bb.Max.x, grab_pos + grab_sz*0.5f));
	window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), 5);
	PositionOfText = grab_bb.Max;
	return value_changed;
}
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
	...
	RenderText(ImVec2(frame_bb.Max.x + label_size.x + 1, frame_bb.Min.y - 0.5f), value_buf);
	RenderText(ImVec2(frame_bb.Max.x, frame_bb.Min.y - 1), label);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, 3.5f);
	if (*v)
	{
		const float check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
		const float pad = ImMax(1.0f, (float)(int)(check_sz / 6.0f));
		window->DrawList->AddRectFilled(check_bb.Min+ImVec2(1,1), check_bb.Max-ImVec2(1,1), GetColorU32(ImGuiCol_CheckMark), 3.5f);
	}
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
	...
	RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, 3.5f);

	if (*current_item >= 0 && *current_item < items_count)
	{
		const char* item_text;
		int llx = ((frame_bb.Max.x - frame_bb.Min.x) - label_size.x) / 2;
		
		if (items_getter(data, *current_item, &item_text))
			RenderTextClipped(ImVec2(frame_bb.Min.x + llx, frame_bb.Min.y), frame_bb.Max, item_text, NULL, NULL);
	}

	if (label_size.x > 0)
		RenderText(ImVec2(frame_bb.Max.x, frame_bb.Min.y + style.FramePadding.y), label);
	...
	if (IsPopupOpen(id))
	{
		...
		if (BeginPopup(label))
		{
			...
			for (int i = 0; i < items_count; i++)
			{
				...
				if (Selectable1(item_text, item_selected))
				{
					SetActiveID(0);
					value_changed = true;
					*current_item = i;
...
bool ImGui::Selectable1(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	int llx = ((bb.Max.x - bb.Min.x) - label_size.x) / 2;
	
	if (flags & ImGuiSelectableFlags_Disabled) PushStyleColor(ImGuiCol_Text, g.Style.Colors[ImGuiCol_TextDisabled]);
	RenderTextClipped(bb.Min, ImVec2(bb.Max.x + llx, bb.Max.y), label, NULL, &label_size);
...
bool ImGui::Selectable1(const char* label, bool* p_selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	if (Selectable1(label, *p_selected, flags, size_arg))
	{
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	int lls = ((bb.Max.x - bb.Min.x)-label_size.x)/2;
	int llx = ((bb.Max.y - bb.Min.y)-(label_size.y / 2)) / 2;

	ImVec2 Joined(bb.Min.x + lls,bb.Min.y);
	if (flags & ImGuiSelectableFlags_Disabled) PushStyleColor(ImGuiCol_Text, g.Style.Colors[ImGuiCol_TextDisabled]);
	RenderTextClipped(Joined, ImVec2(bb.Max.x,bb.Max.y - llx), label, NULL, &label_size);
	if (flags & ImGuiSelectableFlags_Disabled) PopStyleColor();
