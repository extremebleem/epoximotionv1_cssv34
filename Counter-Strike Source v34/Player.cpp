...
bool& C_CSPlayer::bIsScoped()
{
	MakeNetVar(bool, "DT_CSPlayer", "m_bIsScoped");
}
...
{
	MakeNetVar(float, "DT_CSPlayer", "m_flLowerBodyYawTarget");
}
...
int& C_CSPlayer::m_iLifeState()
{
	MakeNetVar( int, XorStr( "DT_BasePlayer" ), XorStr( "m_lifeState" ) );
}
...
bool C_CSPlayer::GetHitboxBounds(int hitbox, Vector3& mins, Vector3& maxs,float Radius)
{
	Matrix3x4 mat[128];
	...
	VectorTransform(box->bbmin - Radius, mat[box->bone], mins);
	VectorTransform(box->bbmax + Radius, mat[box->bone], maxs);
...
float C_CSPlayer::GetHitboxRadius(int hitbox)
{
	Matrix3x4 mat[128];
